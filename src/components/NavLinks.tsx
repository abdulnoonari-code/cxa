...
// and to know which page you are on, which the old server-rendered rail could
// not do at all. Thirty-four links with no active state is a map with no
// "you are here".
...
const FALLBACK = icon(<circle cx="12" cy="12" r="3.2" />)

function Item({ item, pathname, depth }: { item: NavItem; pathname: string; depth: number }) {
  const active = isActive(pathname, item.href)
  const open = isOpen(pathname, item)
  ...
  return (
    <>
      <Link
        ...
        className={`nav-link${active ? ' active' : ''}${depth > 0 ? ' nav-link-child' : ''}`}
      >
        {ICONS[item.icon] ?? FALLBACK}
        <span className="nav-link-label">{item.label}</span>
        {/* A closed parent says how many views it holds, so the ones that
            collapse are advertised rather than hidden. */}
        {children.length > 0 && !open && <span className="nav-count">{children.length}</span>}
      </Link>
      {open &&
        children.map((child) => <Item key={child.href} item={child} pathname={pathname} depth={depth + 1} />)}
    </>
  )
}
      ...
      {NAV.map((section) => (
        <div key={section.label} className="nav-group">
          ...
          {section.items.map((item) => (
            <Item key={item.href} item={item} pathname={pathname} depth={0} />
          ))}
        </div>
      ))}
    </nav>
