...
export default async function ProjectsPage({
  ...
  const purgeBanner =
    purge === 'ok' ? (
      ...
      <div className="card" style={{ borderLeft: '4px solid var(--color-warning-solid, #d97706)', marginBottom: 14 }} role="alert">
        <p style={{ margin: 0, fontSize: 13.5, fontWeight: 600 }}>
          {one('want')} was not deleted — it is your only project.
        </p>
        <p style={{ margin: '5px 0 0', fontSize: 12.5 }}>
          Every screen in this application is scoped to a project. Create the one you want to keep first, then delete
          this one.
        </p>
      </div>
    ) : null
                      ...
                      >
                        ...
                        {projects.length <= 1 && (
                          <p className="text-secondary" style={{ margin: '6px 0 0', fontSize: 12.5 }}>
                            This is your only project, so it cannot be deleted — every screen is scoped to a project
                            and an installation with none is not a clean slate, it is one where nothing works. Create
                            the project you want to keep first, then delete this one.
                          </p>
                        )}
                        <label
                        ...
                        >
                          Type <span className="mono">{p.name}</span> to confirm
                        </label>
                        <input
                          type="text"
                          name="confirm"
