    ...
    screens: [
      ...
      },
      {
        ...
        facts: [
          ...
          { label: 'The list', text: 'Under the drawing is the same equipment as a spreadsheet, and it is the SAME list \u2014 not a copy and not an export. Click a row and the box lights up; click a box and the row does. Double-click a cell to edit it, Shift-click for a range, Ctrl-click to add one, and the arrow keys walk it like any spreadsheet.' },
          { label: 'From Excel', text: 'Copy a block in Excel and press Ctrl+V. It tells you what it would do \u2014 how many cells change, how many were refused and why \u2014 before anything is written. Fill down repeats the top row of a selection through the rest, counting a tag up and keeping its padding, so PDU-A09 becomes PDU-A10 and never PDU-A010. Ctrl+C copies back out.' },
          { label: 'Many at once', text: 'Select several and the panel on the right edits all of them together. Where they differ a field reads \u2014 mixed \u2014 rather than showing the first one\u2019s value, because a box showing one value would flatten the rest the moment you touched it.' },
          { label: 'Careful', text: 'A cell it cannot read is REFUSED and keeps what it had \u2014 it never guesses. A rating typed as 4l5 with a letter in it is not quietly read as 415, because a guessed value reads afterwards exactly like one somebody meant. The three right-hand columns are worked out from the drawing and cannot be typed over.' },
          ...
          { label: 'The palette', text: 'Twenty-two types in five bands \u2014 Sources, Distribution, Load banks, White space, Not electrical. Search the box at the top if you know what you want. Footprints are real: a rack is 600 by 1200, a 2 MVA transformer is 3.2 by 2.2, a 1 MW load bank is 6.1 by 2.5 with five metres of discharge clearance.' },
          { label: 'Careful', text: 'The band an item is in decides how it is counted. Everything under Distribution \u2014 boards, switchgear, transfer switches, UPS, busway, PDUs \u2014 PASSES load rather than consuming it, so its rating is never added to the loads beneath it. Racks, cooling plant and load banks are loads and are. Get that wrong and every cable under it is sized for double.' },
          { label: 'Excel', text: 'The Excel button builds a load bank plan workbook: Basis, LB Calc, Equipment, Cable schedule, Test plan, Findings, and the room plan and single line as pictures on their own sheets. It takes what is on screen, so the drawing does not have to be saved first.' },
          { label: 'Test plan', text: 'One row per thing that has to be proved, sources first. Day, date, line-up, phase and busway are SHADED AND BLANK \u2014 the drawing knows the equipment and the load, it does not know your programme, and an invented date is worse than a blank one because somebody would plan an outage around it. A load bank is not a row: it is the instrument, not the subject, and what it can deliver appears against every row instead.' },
